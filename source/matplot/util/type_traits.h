...
namespace matplot {
    ...
    struct has_value_type {
        ...
        template<class T> static false_type check(...);
        template<class T> static true_type  check(int, typename T::value_type = *C().begin());
    ...
    template <typename C>
    struct has_iterable_value_type {
        typedef long false_type;
        typedef char true_type;

        template<class T> static false_type check(...);
        template<class T> static true_type  check(int, typename T::value_type::value_type = *(C().begin()->begin()));

        enum { value = sizeof(check<C>(0)) == sizeof(true_type) };
    };

    template<typename C>
    constexpr bool has_iterable_value_type_v = has_iterable_value_type<C>::value;
    ...
    struct is_iterable_value
        : public std::integral_constant<bool, is_iterable_v<C> && !has_iterable_value_type_v<C>> {};
    ...
    template <class T>
    using is_string = std::integral_constant<bool, std::is_same_v<std::decay_t<T>,std::string> && std::is_convertible_v<T,std::string>>;

    template<typename C>
    constexpr bool is_string_v = is_string<C>::value;
    ...
    struct is_iterable_pair
            : public std::integral_constant<bool, is_iterable_v<C> && !has_iterable_value_type_v<C> && is_pair_v<typename C::value_type>> {};
    ...
    struct iterable_traits {
        ...
        using value_type = typename std::remove_reference< decltype( std::declval<iterator_type>().operator*() ) >::type;
        static constexpr bool is_map = is_pair<value_type>::value;
        using key_type = std::conditional_t<is_map, typename value_type::first_type, value_type>;
        using mapped_type = std::conditional_t<is_map, typename value_type::second_type, value_type>;
    };
}
