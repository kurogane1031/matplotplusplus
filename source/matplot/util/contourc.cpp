//
// Adapted from https://github.com/matplotlib/matplotlib/blob/master/src/_contour.cpp
//
...
// Note that EXISTS_NE_CORNER(quad) is equivalent to BOUNDARY_SW(quad), etc.
    ...
    QuadContourGenerator::QuadContourGenerator(const CoordinateArray &x,
            ...
            : _x(x),
              _y(y),
              _z(z),
              _nx(static_cast<long>(_x[0].size())),
              _ny(static_cast<long>(_x.size())),
              _n(_nx * _ny),
              ...
              _cache(std::vector<CacheItem>(_n)),
              _parent_cache(_nx,
    ...
    void QuadContourGenerator::append_contour_to_vertices_and_codes(
        ...
        for (Contour::iterator line_it = contour.begin(); line_it != contour.end(); ++line_it) {
                ...
                for (children_it = children.begin(); children_it != children.end(); ++children_it) {
                    ...
                    for (point = child.begin(); point != child.end(); ++point) {
                        if (is_valid_point(point->x,point->y)) {
                            vertices_list.first.emplace_back(point->x);
                            vertices_list.second.emplace_back(point->y);
                            codes_list.emplace_back((never_emplaced_anything ? MOVETO : LINETO));
    ...
    std::pair<QuadContourGenerator::vertices_list_type, QuadContourGenerator::codes_list_type>
    QuadContourGenerator::create_filled_contour(const double &lower_level, const double &upper_level) {
    ...
    bool QuadContourGenerator::start_line(
            vertices_list_type& vertices_list, long quad, Edge edge, const double &level) {
    ...
    find_closest_point_on_path(const std::vector<double>& xs, const std::vector<double>& ys, double px, double py) {
        if (xs.size() == 1) {
            ...
            return std::make_tuple(m, std::make_pair(xs[0],ys[0]), std::make_pair(0, 0));
        }
        ...
        vector_1d rel_projs = transform(a, [](double a) { return truncate(a, 0, 1); });
        ...
        // Squared distances.
        vector_1d d2s = transform(projxs, projys, [&](double x, double y) { return pow(x - px, 2) + pow(y - py, 2); });

        size_t imin = std::min_element(d2s.begin(), d2s.end()) - d2s.begin();
        return std::make_tuple(d2s[imin], std::make_pair(projxs[imin], projys[imin]), std::make_pair(imin, imin+1));
    }

    contour_line_type
    contour_line(const vector_2d& X, const vector_2d& Y, const vector_2d& Z, double level) {
        ...
        // until we get sometime that makes sense (something at least not out of range).
        // We then make sure this contour line matches the next contour line.
        double x_min = min(X);
        double x_max = max(X);
        double y_min = min(X);
        double y_max = max(X);
    ...
    std::vector<contour_line_type>
    contourc(const vector_2d& x, const vector_2d& y, const vector_2d& z, size_t n_levels) {
        double zmin = z[0][0];
        double zmax = z[0][0];
    ...
    std::vector<contour_line_type>
    contourc(const vector_2d& z, size_t n_levels) {
        auto [x,y] = meshgrid(iota(1,1,z[0].size()), iota(1,1,z.size()));
