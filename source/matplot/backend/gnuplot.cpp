...
namespace matplot::backend {
    ...
    bool gnuplot::is_interactive() {
        return output_.empty();
    }

    const std::string &gnuplot::output() {
        return output_;
    }

    const std::string &gnuplot::output_format() {
        return terminal_;
    }
    ...
    bool gnuplot::output(const std::string &filename, const std::string &format) {
    ...
    unsigned int gnuplot::width() {
        return position_[2];
    }

    unsigned int gnuplot::height() {
        return position_[3];
    }
    ...
    void gnuplot::width(unsigned int new_width) {
        ...
        if (terminal_has_position_option(terminal_)) {
            run_command("set terminal " + terminal_ + " position " + num2str(position_[0]) + "," + num2str(position_[1]));
        }
        if (terminal_has_size_option(terminal_)) {
            run_command("set terminal " + terminal_ + " size " + num2str(position_[2]) + "," + num2str(position_[3]));
        }
    }
    ...
    unsigned int gnuplot::position_x() {
        return position_[0];
    }

    unsigned int gnuplot::position_y() {
        return position_[1];
    }
    ...
    void gnuplot::new_frame() {
        if (!consumes_gnuplot_commands()) {
            throw std::logic_error("There is no function to start new_frame in the gnuplot backend yet");
        } else {
            throw std::logic_error("This backend has no function new_frame because it is based on gnuplot commands");
        }
    }

    bool gnuplot::render_data() {
    ...
    std::pair<int,int> gnuplot::gnuplot_version() {
        ...
        const bool dont_know_gnuplot_version = version == std::pair<int,int>({0,0});
        if (dont_know_gnuplot_version) {
            std::string version_str = run_and_get_output("gnuplot --version 2>&1");
            ...
            const bool still_dont_know_gnuplot_version = version == std::pair<int,int>({0,0});
    ...
    bool gnuplot::terminal_has_title_option(const std::string& t) {
        constexpr std::string_view whitelist[]={"qt", "aqua", "caca", "canvas", "windows","wxt","x11"};
    ...
    bool gnuplot::terminal_has_size_option(const std::string& t) {
        ...
        constexpr std::string_view whitelist[]={"qt", "aqua", "caca", "canvas", "eepic", "emf", "gif", "jpeg", "pbm", "png", "sixelgd", "tkcanvas", "windows", "wxt", "svg"};
        return std::find(std::begin(whitelist), std::end(whitelist), t) != std::end(whitelist);
    }
    ...
    bool gnuplot::terminal_has_enhanced_option(const std::string& t) {
        constexpr std::string_view whitelist[]={"canvas", "postscript","qt","aqua","caca","canvas","dumb","emf", "enhanced", "jpeg", "pdf", "pdfcairo","pm", "png", "pngcairo", "postscript", "sixelgd", "tkcanvas", "windows", "wxt","x11", "ext", "wxt"};
        return std::find(std::begin(whitelist), std::end(whitelist), t) != std::end(whitelist);
    }

    bool gnuplot::terminal_has_color_option(const std::string& t) {
        constexpr std::string_view whitelist[]={"postscript", "aifm", "caca", "cairolatex", "context", "corel", "eepic", "emf", "epscairo", "epslatex", "fig", "lua tikz", "mif", "mp", "pbm", "pdf", "pdfcairo", "pngcairo", "postscript", "pslatex", "pstex", "tgif", "windows"};
        return std::find(std::begin(whitelist), std::end(whitelist), t) != std::end(whitelist);
    }
    ...
    bool gnuplot::terminal_has_font_option(const std::string& t) {
        ...
        constexpr std::string_view blacklist[]= {"dxf", "eepic", "emtex", "hpgl", "latex", "mf", "pcl5", "pslatex", "pstex", "pstricks", "qms", "tek40xx", "tek410x", "texdraw", "tkcanvas", "vttek", "xterm", "jpeg", "dumb", "tpic", "sixelgd", "png", "lua", "jpeg", "epscairo", "epslatex", "canvas", "dumb", "dxy800a", "emxvga", "pdfcairo", "cgi", "gif", "gpic", "grass", "hp2623a", "hp2648", "hp500c", "hpgl", "pcl5", "hpljii", "hppj", "imagen", "linux", "lua", "lua tikz", "mf", "mif", "mp", "pbm", "pm", "qms", "regis", "svga", "texdraw", "tikz", "tpic", "vgagl", "vws", "pdf"};
